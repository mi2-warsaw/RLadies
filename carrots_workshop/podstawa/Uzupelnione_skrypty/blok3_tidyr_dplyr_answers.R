...
#a)
ranking_szerokie <- spread(ranking, tydzien, ranking)

#b)
ranking_szerokie_ur <- unite(ranking_szerokie, data_urodzenia, c("dzien_ur", "miesiac_ur"), sep = "-")

#c)

ranking_rozdzielone <- separate(ranking_szerokie_ur,x, c("id","imie","wiek"), sep="\\.")
...
df <- filter(apartments, district=="Wola"& surface > 50)
...
#a)
apartments2 <- summarise(apartments_gr, number = mean(n.rooms),mean_transaction = mean(transaction.price))

#b)
apartments_type <- group_by(apartments, type)
apartments3 <- summarise(apartments_type, mean_transaction = mean(transaction.price))
...
#a)
Srodmiescie <- filter(apartments, district =="Srodmiescie")
df<- arrange(Srodmiescie, m2.price)

#b) 
apartments5 <- mutate(apartments, EUR.price = transaction.price/4)
...
apartments_sr_pow3 <- apartments %>% 
                        group_by(district) %>% 
                        summarise(mean_surface = mean(surface))

#Zadanie 5
#Używając operatora %>% ponownie zrób zadanie 3b : Znajdź średnią ceną za mieszkanie grupując dane ze względu na zmienną "type"


apartments_pow <- apartments %>% 
                  group_by(type) %>% 
                  summarise(mean_trans_price = mean(transaction.price))
