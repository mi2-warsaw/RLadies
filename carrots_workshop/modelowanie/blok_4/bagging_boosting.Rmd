...
mm_boston_train_label <- Boston$crim[train_ids]
mm_boston_test_label <- Boston$crim[-train_ids]
```
...
tmp <- lm(crim ~., boston_train)
RMSE(predict(tmp, boston_test), boston_test$crim)
```
...
```{r}
importance_matrix <- xgb.importance(colnames(mm_boston_train), model = xg_model)
importance_matrix
```
